...
如今互联网产品如如雨后春笋般层出不穷,程序员们每天都要面对变化无穷的需求，来满足产品的需要，他们不得不加班加点，来完成他们的使命，由最初的web1.0,web2.0,3.0到现在的移到互联网，可谓是变化莫测。eBSite网站工程应运而生,为解决变化无穷的需求PC互联产品与移动互联网产品而来。如果您是程序员，可以把她当成是一个基础框架，在这个基础之上，你能轻而易举的开发出你的客户想要的互联网产品，如果您不是程序员，或者只是一个普通的站长，你可以把她当成是一个比传统CMS更高端的产品来使用，因为官方已经为您集成了好了，资讯发布系统(新闻发布系统),网上商城系统，互联问答系统，论坛(bbs),个人空间及在线考试系统等，这些系统集成在eBSite，您可以单独使用，也可以以站群的方式使用.

2008年正式起名eBSite网站工程系统，简称为eBSite，最后被应用到大量的互联网产品之中，所以经历过大流量，大负载的，和安全性能的考验。

# eBSite网站工程的新特性:

- ### 1.PC站点与移动站点轻松对接。
如今移动互联飞速发展，应对手机平板访问的网站少之又少，大量都是靠百度或移动浏览器转换后的页面展示给用户，这样的用户体验及其之差，同时不能实现更加复杂的功能。eBSite可以轻松制作或开发复杂的大型移动站点。同时提供一个良好的用户体验前端架构。

- ### 2.成熟模块机制
过去的cms也提供模块，但由于过于复杂，和安装不方便，使这些功能往往只有官方会使用，eBSite模块从开发到安装，整个体验有着非常好的用户体验，套路式开发，并提供非常便捷的模板，同时，官方还为用户提供模块生成器，让您的模块开发过程更加傻瓜式，将为您省去80%的重复工作。
  
- ### 3.先进的插件与动态组件机制
虽然插件已经不是什么新鲜的东西，但是eBSite的插件在web上的应用表现得与众不同,提供可以配置式开发插件,安装方便,开关灵活,并且官方提供了大量常用插件，你只要按需安装即可。动态组件与插件原理相同，只不过动态组件更加轻巧，它没有规范，意味着它能实现任何想做的事，由于他是在程序启动时动态编译，所以叫动态组件。

- ### 4.站群更加出色
  您可以配置同站下的站群，也可以配置出不同域下的站群，由于eBSite在用户系统设计上已经考虑到这一点，所以eBSite的用户系统是独立的，数据库也是独立的，无论您有多少个站点，都可以使用同一个用户系统，意味着所有的用户数据在所有站点下都是只有一份，如积分，金额等，您不必担心不同站点之间用户信息混乱的问题。

- ### 5.丰富的扩展事件
  对于二次开发人员来说，这个是eBSite最值得学习的一部分，官方开发的商城，问答，论坛，考试系统都是离不开这些可扩展的事件，这些事件应用在模块里，你可以轻而易举的开发出任何您能想象得出的互联网产品，过去几十人才能做的事件，现在你可以在一个人在很短的时间里完成。

- ### 6.简单易用的皮肤(模板与样式)
  eBSite皮肤由模板与样式组合，有了模板，可以让网页的展示更加灵活，eBSite为模板的制作提供了丰富的API。

- ### 7.出色的部件
  部件相当于一个代码块，部件的开发很简单，只要实现官方提供的接口即可完成，部件要配件模板来使用，可以应用于模板里的某一个地方，比如要在模板中调用最新新闻，或今日排行榜数据等，这些都可以直接在后台创建官方已经开发好的部件，只要做简的配置即可，部件还支持模板，所以输出的html代码是完全可定义的。部件除了调用数据，还能实现更加复杂的业务。

- ### 8.强大的模型与模型控件。
  过去的CMS都支持模型,但都只是只支持内容模型,eBSite支持的模型包括，内容模型，分类模型，用户模型，与表单模型,eBSite的模型可以自定义模型展现字段的中文名称，还能设置不同字段对应的输入控件，更加值得一提的是，这些模型控件你还能开发，并且安装到eBSite中来，传统CMS模型非常死板，而eBSite在这方面有所超越，这些模型控件的可开发特性，意味着你可以制作更多加复杂的模型。
...
```
2022-4-12:
1、升级asp.net mvc 5 框架到最新版本
2、更新了前端框架，如 boostrap,jquery等。
3、.net 框架升级为 4.8 
4、web api支持跨域调用设置
5、添加图片批量上传控件与单图片上传控件 UploadImg
6、添加文件批量上传与单文件上传控件 WebUploaderFile
```
