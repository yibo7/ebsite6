...
//[assembly: PreApplicationStartMethod(typeof(EbSite.Modules.BBS.AppStart), "Start")]
namespace EbSite.Modules.BBS
{
    public class AppStart: ModuleStartInit
    {
        ...
        public  void Start()
        {
            ...
            EBSiteEvents.SessionStart += new EventHandler<EventArgs>(Session_Start);



            EbSite.Log.Factory.GetInstance().InfoLog("EbSite.Modules.BBS模块启动完成,成功监听Application_BeginRequest事件");
        ...
        /// <param name="e"></param>
        private  void On_ClassUpdatedEvent(object sender, UpdatedClassEventArgs e)
        {
        ...
        private  void On_ClassAddedEvent(object sender, AddedClassEventArgs e)
        {
            if (e.ID > 0 && e.Entity.SiteID == SiteID)
            {
                string tablename = Comm.GetPostTableNamePre(e.ID);//表名称
                //在创建分类的时候，为分类生成一个回复帖子的表
                int isok = TopicReplies.Instance.TopicReplies_Copy(tablename);
        ...
        private  void On_ClassPageLoadingEvent(object sender, ClassPageLoadingEventArgs e)
        {
            if (e.SiteID == SiteID)
            {
                //验证分类访问权限
        ...
        private  void On_ClassPageLoadEvent(object sender, ClassPageLoadEventArgs e)
        {
            if (e.SiteID == SiteID)
            {
