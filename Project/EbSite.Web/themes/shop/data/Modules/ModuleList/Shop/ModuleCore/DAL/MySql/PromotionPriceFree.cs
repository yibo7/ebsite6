	...
	public partial class Shop
	{
		...
		public int PromotionPriceFree_Add(Entity.PromotionPriceFree model)
		{
			...
			MySqlParameter[] parameters = {
					...
					new MySqlParameter("?FreightFree", MySqlDbType.Bit,1),
					new MySqlParameter("?ServiceFree", MySqlDbType.Bit,1),
					new MySqlParameter("?PayFee", MySqlDbType.Bit,1)};
		...
		public List<Entity.PromotionPriceFree> PromotionPriceFree_GetListPages(int PageIndex, int PageSize, string strWhere, string Fileds, string oderby,out int RecordCount)
        {
            ...
            RecordCount = PromotionPriceFree_GetCount(sbSql.ToString());
            List<Entity.PromotionPriceFree> list = new List<Entity.PromotionPriceFree>();
            string strSql = SplitPages.GetSplitPagesMySql(DB, "PromotionPriceFree", PageSize, PageIndex, Fileds, "id", oderby, sbSql.ToString(), sPre);
		...
		public Entity.PromotionPriceFree PromotionPriceFree_ReaderBind(IDataReader dataReader)
		{
			...
			if(ojb != null && ojb != DBNull.Value)
			{
                ...
                {
                    model.FreightFree = true;
                }
                else
                {
                    model.FreightFree = false;
                }
			}
			...
			if(ojb != null && ojb != DBNull.Value)
			{
                if (ojb.ToString().Equals("1") || ojb.ToString().Equals("true"))
                {
                    model.ServiceFree = true;
                }
                else
                {
                    model.ServiceFree = false;
                }
			}
