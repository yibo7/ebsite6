...
namespace EbSite.Modules.Shop.ModuleCore.DAL.SqlServer
{
	...
	public partial class Shop
	{
		...
		{
			return DB.GetMaxID("id", string.Format("{0}CouponItems",sPre)); 
		}
		...
		/// 增加一条数据
		/// </summary>
		public int CouponItems_Add(EbSite.Entity.CouponItems model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 更新一条数据
		/// </summary>
        public void CouponItems_Update(EbSiteEntity.CouponItems model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		{
			List<Entity.CouponItems> list = new List<Entity.CouponItems>();
			using (IDataReader dataReader = SplitPages.GetListPages_SP(DB,"CouponItems", PageSize, PageIndex, Fileds, "id", oderby, strWhere, out RecordCount,sPre))
			{
				while (dataReader.Read())
				{
					list.Add(CouponItems_ReaderBind(dataReader));
