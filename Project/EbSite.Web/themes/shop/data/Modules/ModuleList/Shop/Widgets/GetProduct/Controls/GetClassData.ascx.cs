        ...
        protected List<Entity.NewsContent> GetData()
        {
            ...
            if (lst == null)
            {
                ...
                if (DataModel == DataListType.GoodList)
                {
                    ...
                    lst = BLL.NewsContent.GetListGood(iTop, iClassID, IsGetSub, IsHaveImg, Fields, SiteID);
                }
                else if (DataModel == DataListType.AllHot)
                {
                    lst = BLL.NewsContent.GetListHot(iClassID, iTop, "", IsGetSub, IsHaveImg, Fields, SiteID);
                }
                else if (DataModel == DataListType.WeekHot)
                {
                    lst = BLL.NewsContent.GetListHot(iClassID, iTop, "w", IsGetSub, IsHaveImg, Fields, SiteID);
                }
                else if (DataModel == DataListType.Adv)
                {
                    lst = BLL.NewsContent.GetListHot(iClassID, iTop, "adv", IsGetSub, IsHaveImg, Fields, SiteID);
                }
                else if (DataModel == DataListType.DayHot)
                {
                    lst = BLL.NewsContent.GetListHot(iClassID, iTop, "d", IsGetSub, IsHaveImg, Fields, SiteID);
                }
                else if (DataModel == DataListType.MonthHot)
                {
                    lst = BLL.NewsContent.GetListHot(iClassID, iTop, "m", IsGetSub, IsHaveImg, Fields, SiteID);
                }
                else if (DataModel == DataListType.CommentHot)
                {
                    lst = BLL.NewsContent.GetListHot(iClassID, iTop, "ch", IsGetSub, IsHaveImg, Fields, SiteID);
                }
                else if (DataModel == DataListType.FavorableHot)
                {
                    lst = BLL.NewsContent.GetListHot(iClassID, iTop, "fh", IsGetSub, IsHaveImg, Fields, SiteID);
                }
                else
                {
                    lst = BLL.NewsContent.GetListNewOfNewsClass(iClassID, iTop, IsGetSub, IsHaveImg, Fields, SiteID);
                }
