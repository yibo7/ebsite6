...
{
    public class ExClassPageLoadEvent
    {
        ...
        public ExClassPageLoadEvent(HttpContext Context, int classid)
        {
            _context = Context;
        ...
        public string SpecialUrl(int spParentID, int Brand, string orderby, string valusTag)
        {
            ...
            if (spParentID > 0)
            {
                Url = EbSite.Base.Host.Instance.GetClassHref(ClassID, 1, SettingInfo.Instance.GetSiteID) + "?special=" + spParentID + iEUrl;
            }
            else
            {
                if (iEUrl.Length > 0)
                ...
                if (iEUrl.Length == 0)
                {
                    Url = EbSite.Base.Host.Instance.GetClassHref(ClassID, 1, SettingInfo.Instance.GetSiteID);
                }
                else
                {
                    Url = EbSite.Base.Host.Instance.GetClassHref(ClassID, 1, SettingInfo.Instance.GetSiteID) + "?" + iEUrl;
                }
        ...
        public List<ListItemModelEx> SpecialList(int pid)
        {
            ...
            foreach (var childTemp in arry)
            {
                ...
                md.Url = EbSite.Base.Host.Instance.GetClassHref(ClassID, 1, SettingInfo.Instance.GetSiteID) + "?special=" + childTemp.id + iEUrl;
                #endregion
        ...
        public List<ListItemModelEx> BrandList(string value)
        {
            ...
            foreach (var childTemp in arry)
            {
                ...
                md.Url = EbSite.Base.Host.Instance.GetClassHref(ClassID, 1, SettingInfo.Instance.GetSiteID) + "?brand=" + childTemp + iEUrl;
                #endregion
        ...
        public List<ListItemModelEx> SkuBind(int value)
        {
            ...
            foreach (var childTemp in xls)
            {
                ...
                if (CheckSku(Convert.ToInt32(childTemp.TypeNameValueID)))
                {
                    ...
                    {
                        md.Url = EbSite.Base.Host.Instance.GetClassHref(ClassID, 1, SettingInfo.Instance.GetSiteID) + "?valueStr=" + md.ID + "_" + md.Value;
                    }
                    else if (RawUrl.ToLower().IndexOf("valuestr") == -1 && (RawUrl.ToLower().IndexOf("brand") > -1 || RawUrl.ToLower().IndexOf("special") > -1 || RawUrl.ToLower().IndexOf("orderby") > -1))
                    {
                        ...
                        if (iEUrl.Length == 0)
                        {
                            md.Url = EbSite.Base.Host.Instance.GetClassHref(ClassID, 1, SettingInfo.Instance.GetSiteID);
                        }
                        else
                        {
                            md.Url = EbSite.Base.Host.Instance.GetClassHref(ClassID, 1, SettingInfo.Instance.GetSiteID) + "?" + iEUrl + "&valueStr=" + md.ID + "_" + md.Value;
                        }
                    ...
                    else if (RawUrl.ToLower().IndexOf("valuestr") > -1 && RawUrl.ToLower().IndexOf("brand") == -1 && RawUrl.ToLower().IndexOf("special") == -1 && RawUrl.ToLower().IndexOf("orderby") == -1)
                    {
                        ...
                        md.Url = EbSite.Base.Host.Instance.GetClassHref(ClassID, 1, SettingInfo.Instance.GetSiteID) + "?valueStr=" + strurl + md.ID + "_" + md.Value;
                    ...
                    {   //?brand=1&valueStr=2_6-1_1
                        ...
                        {
                            md.Url = EbSite.Base.Host.Instance.GetClassHref(ClassID, 1, SettingInfo.Instance.GetSiteID) + "?" + iEUrl;
                        }
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        md.Url = EbSite.Base.Host.Instance.GetClassHref(ClassID, 1, SettingInfo.Instance.GetSiteID) + "?valueStr=" + valusTag + "-" + md.ID + "_" + md.Value + iEUrl;
                    }
    ...
    public class ExIndexPageLoadEvent
    {
        public ExIndexPageLoadEvent()
        {

        }
        #region 首页楼层repeater 扩展
        public static void rpList_ItemDataBound(object sender, System.Web.UI.WebControls.RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ModuleCore.Entity.FloorSet drData = (ModuleCore.Entity.FloorSet)e.Item.DataItem;

                #region rpFloorLeftAd
                if (!Equals(drData, null))
                {
                    List<ModuleCore.Entity.FloorLeftAd> lsSmall = ModuleCore.BLL.FloorLeftAdInfo.Instance.FillList();
                    List<ModuleCore.Entity.FloorLeftAd> NlsSmall =
                        (from i in lsSmall where i.FloorSetId == drData.id select i).ToList();
                    Repeater llClassList = (Repeater)e.Item.Controls[0].FindControl("rpFloorLeftAd");

                    if (!Equals(llClassList, null))
                    {
                        llClassList.DataSource = NlsSmall;
                        llClassList.DataBind();
                    }
                }
                #endregion

                #region rpFloorSmallClass
                if (!Equals(drData, null))
                {
                    List<ModuleCore.Entity.FloorSmallClass> lsSmall = ModuleCore.BLL.FloorSmallClassInfo.Instance.FillList();
                    List<ModuleCore.Entity.FloorSmallClass> NlsSmall =
                        (from i in lsSmall where i.FloorSetId == drData.id select i).ToList();
                    Repeater llClassList = (Repeater)e.Item.Controls[0].FindControl("rpFloorSmallClass");
                ...
                #endregion
                #region rpFloorBigClass
                if (!Equals(drData, null))
                {
                    List<ModuleCore.Entity.FloorBigClass> lsSmall = ModuleCore.BLL.FloorBigClassInfo.Instance.FillList();
                    List<ModuleCore.Entity.FloorBigClass> NlsSmall =
                        (from i in lsSmall where i.FloorSetId == drData.id select i).ToList();
                    Repeater llClassList = (Repeater)e.Item.Controls[0].FindControl("rpFloorBigClass");
                ...
                #endregion
                #region rpFloorProducts

                if (!Equals(drData, null))
                {
                    List<ModuleCore.Entity.FloorProducts> lsSmall = ModuleCore.BLL.FloorProductsInfo.Instance.FillList();
                    List<ModuleCore.Entity.FloorProducts> md =
                         (from i in lsSmall where i.FloorSetId == drData.id select i).ToList();
                    if (md.Count > 0)
                    {
                        List<EbSite.Entity.NewsContent> LsProduct =
                            EbSite.Base.AppStartInit.NewsContentInstDefault.GetListArray("id in(" + md[0].ProductIds + ")", 0,
                                                                "",
                                                                "id,SmallPic,NewsTitle,Annex16,Annex2",
                                                                SettingInfo.Instance.GetSiteID);
                        Repeater llClassList2 = (Repeater)e.Item.Controls[0].FindControl("rpFloorProducts");
                        if (!Equals(llClassList2, null))
                        {
                            llClassList2.DataSource = LsProduct;
                ...
                #region rpFloorRightBrand
                if (!Equals(drData, null))
                {
                    List<ModuleCore.Entity.FloorRightBrand> lsSmall = ModuleCore.BLL.FloorRightBrandInfo.Instance.FillList();
                    List<ModuleCore.Entity.FloorRightBrand> NlsSmall =
                        (from i in lsSmall where i.FloorSetId == drData.id select i).ToList();
                    Repeater llClassList = (Repeater)e.Item.Controls[0].FindControl("rpFloorRightBrand");
                    if (!Equals(llClassList, null))
                    {
                        llClassList.DataSource = NlsSmall;
                        llClassList.DataBind();
                ...
                #region rpFloorRightAd
                if (!Equals(drData, null))
                {
                    List<ModuleCore.Entity.FloorRightAd> lsSmall = ModuleCore.BLL.FloorRightAdInfo.Instance.FillList();
                    List<ModuleCore.Entity.FloorRightAd> NlsSmall =
                        (from i in lsSmall where i.FloorSetId == drData.id select i).ToList();
                    Repeater llClassList = (Repeater)e.Item.Controls[0].FindControl("rpFloorRightAd");
                    if (!Equals(llClassList, null))
                    {
                        llClassList.DataSource = NlsSmall;
                        llClassList.DataBind();
    ...
    public class ListItemModelEx : ListItemModel
    {
        public string Url { get; set; }
        public string StyleBg { get; set; }
    }
}                                       