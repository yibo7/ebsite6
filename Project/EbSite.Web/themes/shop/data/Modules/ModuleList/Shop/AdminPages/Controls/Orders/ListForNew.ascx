<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="ListForNew.ascx.cs" Inherits="EbSite.Modules.Shop.AdminPages.Controls.Orders.ListForNew" %>
        ...
        <XS:Repeater ID="rpList" runat="server" >
            <HeaderTemplate>
                <tr class="GridViewHeader">
                    ...
                    <th scope="col">订单实收款(元)</th><th scope="col">订单状态</th><th>打印状态</th><th>标注</th><th>下单时间</th><th>操作</th>
                    <th scope="col"><input id='chAll' onclick='on_check(this)'  type=checkbox /></th>
                </tr>
            </HeaderTemplate>
            <ItemTemplate>
                <tr tid="<%# Eval("ID") %>"> 
                    <td style="width:180px;"><%# Eval("orderid") %>&nbsp;<%# Eval("groupid")==null?"":"<span style='color:blue;font-weight:bold;'>(团)</span>" %><%# Eval("RefundAmount") == null ? "" : "<span style='color:red;font-weight:bold;cursor:pointer;' onclick='RefundOrder(this)'>(退)</span>" %></td>
                    <td><%#Eval("username")%></td>
                    ...
                    <td><div><%# base.ParseOrderState(Eval("OrderStatus").ToString())%></div><div><a href="javascript:void(0);" onclick="ViewOrderDetail(this)">详情</a>&nbsp;&nbsp;<%# Eval("OrderStatus").ToString().Equals("3")?"<a onclick=\"RefundOrder(this)\">退款</a>":"" %></div>
                    ...
                    <td><%#Eval("OrderAddDate")%></td>
                    <td> <a href='javascript:void(0);' title='只有通过审核后才能发货哦'  onclick='ApprovedOrder(this)'>通过审核</a> &nbsp;<a onclick="MarkOrder(this)">标注</a>
                    </td>
