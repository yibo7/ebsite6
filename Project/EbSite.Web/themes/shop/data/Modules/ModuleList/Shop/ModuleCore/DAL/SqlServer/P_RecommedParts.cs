    ...
    public partial class Shop
	{
		...
		public int P_RecommedParts_Add(Entity.P_RecommedParts model)
		{
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@PartsID", SqlDbType.NChar,10),
					new SqlParameter("@OrderiD", SqlDbType.NChar,10),
					new SqlParameter("@PartsAvatarSmall", SqlDbType.NVarChar,200),
					new SqlParameter("@PartsName", SqlDbType.NVarChar,200)};
		...
		{
			List<Entity.P_RecommedParts> list = new List<Entity.P_RecommedParts>();
			using (IDataReader dataReader = SplitPages.GetListPages_SP(DB,"P_RecommedParts", PageSize, PageIndex, Fileds, "id", oderby, strWhere, out RecordCount,sPre))
			{
				while (dataReader.Read())
				{
					list.Add(P_RecommedParts_ReaderBind(dataReader));
