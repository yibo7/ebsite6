...
namespace EbSite.Modules.Shop.ModuleCore.DAL.SqlServer
{
	...
	public partial class Shop
	{
		...
		{
			return DB.GetMaxID("id", string.Format("{0}OrderOptionValue",sPre)); 
		}
		...
		{
			List<Entity.OrderOptionValue> list = new List<Entity.OrderOptionValue>();
			using (IDataReader dataReader = SplitPages.GetListPages_SP(DB,"OrderOptionValue", PageSize, PageIndex, Fileds, "id", oderby, strWhere, out RecordCount,sPre))
			{
				while (dataReader.Read())
				{
					list.Add(OrderOptionValue_ReaderBind(dataReader));
