	...
	public partial class Shop
	{
		...
		public int NormsValue_Add(Entity.NormsValue model)
		{
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@NormsValueName", SqlDbType.VarChar,50),
					new SqlParameter("@NormsIco", SqlDbType.VarChar,250),
		...
		{
			List<Entity.NormsValue> list = new List<Entity.NormsValue>();
			using (IDataReader dataReader = SplitPages.GetListPages_SP(DB,"NormsValue", PageSize, PageIndex, Fileds, "ID", oderby, strWhere, out RecordCount,sPre))
			{
				while (dataReader.Read())
				{
					list.Add(NormsValue_ReaderBind(dataReader));
