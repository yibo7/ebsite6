...
namespace EbSite.Modules.BmAsk.ModuleCore.Pages
{
    public class mcontent : EbSite.Base.Page.CustomPage
    {
        ...
        protected DataTable Model = new DataTable();
        /// <summary>
        /// 问题ID
        /// </summary>
        ...
        protected EbSite.Base.EntityAPI.MembershipUserEb UserInfos
        {
            get
            {
                Base.EntityAPI.MembershipUserEb _UserInfos = MembershipUserEb.Instance.GetEntity(Model.Rows[0]["UserID"].ToString());
                if (_UserInfos != null)
                    return _UserInfos;
                else
                {
        ...
        {
            return EbSite.BLL.NewsClass.GetNav(Nav,int.Parse( Model.Rows[0]["ClassID"].ToString()), false, GetSiteID);
        }
        protected void Page_Load(object sender, EventArgs e)
        {


           // Model = EbSite.BLL.NewsContent.GetModel(iRequestID);

            DataSet ds = ModuleCore.BLL.Answers.Instance.GetDataArticle(iRequestID);
            if (ds != null && ds.Tables.Count > 0)
            {
                ...
                {
                    str = string.Format(tempOne, EbSite.Core.UBB.Ubb2Html(Model.Rows[0]["ContentInfo"].ToString()));
                }
                else
                {
                    str = string.Format(tempOne, Model.Rows[0]["ContentInfo"].ToString());
                }
        ...
        private void inithead()
        {
            ...
            GetSeoWord(ref seoClassTitle, ref seoKeyWord, ref seoDes,Model.Rows[0]["NewsTitle"].ToString(), Model.Rows[0]["ClassName"].ToString(),int.Parse( Model.Rows[0]["ClassID"].ToString()));
        ...
        //private void BestAnswerDataBing()
        //{
        //    // string AskID = Request.QueryString["id"];
        //    string sqlStr = " IsAdoption={0} AND QID={1} ";
        //    sqlStr = string.Format(sqlStr, 1, iRequestID);
        ...
        protected int count;
        //private void AskDataBing()
        //{

        //    string key = Model.Annex4;
        //    string sqlStr = "";
        ...
        //        sqlStr = string.Format(sqlStr, 0, iRequestID);
        //    }
        //    if (key == "1")
        //    {
        //        sqlStr += " IsDel={0} AND QID={1} and IsAdoption={2} ";
        //        sqlStr = string.Format(sqlStr, 0, iRequestID, 0);
        //    }
        //    if (key == "2")
        //    {
        //        sqlStr += " IsDel={0} AND QID={1} and IsAdoption={2} ";
        ...
        public void OutTimeManage(int cid)
        {
            ...
            if (Model.Rows[0]["Annex4"].ToString()== Convert.ToInt32(SystemEnum.AskState.NoSolve).ToString())
            {
                long i = Core.Strings.cConvert.DateDiff("h", DateTime.Now, Convert.ToDateTime(Model.Rows[0]["Annex9"].ToString()));
                if (i <= 0)
                {
                    //过期了
                    EbSite.Entity.NewsContent md = EbSite.BLL.NewsContent.GetModel(iRequestID);
        ...
        protected string SetBestAnswer(int askUid, string AnswerUserID, int aid, int isapproved)
        {
            ...
            string sAnswer = "";
            string sZhuiWen = "";
            string key = Model.Rows[0]["Annex4"].ToString();
            if (isapproved == 1) //通过审核  才出现 设为最佳答案 和 追问
            {
                ...
                if (key == "0")
                {
                    ...
                    if (currentUid == askUid)
                    {
                        ...
                        str = string.Format(str, AnswerUserID, aid, Model.Rows[0]["UserNiName"].ToString());
                    }
