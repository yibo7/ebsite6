...
namespace EbSite.Modules.Ask.ModuleCore.DAL.SqlServer
{
	...
	public partial class Ask
	{
		...
		public int Comment_Add(Entity.Comment model)
		{
			...
			SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4),
					new SqlParameter("@AnsewerID", SqlDbType.Int,4),
					new SqlParameter("@ContentTxt", SqlDbType.Text),
		...
		{
			List<Entity.Comment> list = new List<Entity.Comment>();
			using (IDataReader dataReader = SplitPages.GetListPages_SP(DB,"Comment", PageSize, PageIndex, Fileds, "id", oderby, strWhere, out RecordCount,sPre))
			{
				while (dataReader.Read())
				{
					list.Add(Comment_ReaderBind(dataReader));
