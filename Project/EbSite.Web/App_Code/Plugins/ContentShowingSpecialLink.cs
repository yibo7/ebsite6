...
/// </summary>
[Extension("在文章展示适配专题链接", "1.5", "<a href=\"http://www.ebsite.net\">小菜菜</a>")]
    public class ContentShowingSpecialLink
{
    static protected ExtensionSettings _settings = null;
    static ContentShowingSpecialLink()
        {
            
            EbSite.Base.EBSiteEvents.ContentShowEvent += new EventHandler<ContentShowEventArgs>(Post_Serving);

            string sSettingsName = "ContentShowingSpecialLink";//(注意，注意)要与类名相同,否则无法生成相关配置
    ...
    private static void Post_Serving(object sender, ContentShowEventArgs e)
        ...
        if (e.ContentID > 0&&!string.IsNullOrEmpty(e.ShowInfo))
            ...
            if (sp.Count > 1)
            {
                ...
                string sWhere = string.Format(" SpecialName in('{0}')", sWords);

                List<EbSite.Entity.SpecialClass> lst = DbProviderCms.GetInstance().SpecialClass_GetListArray(sWhere, iKeyWordNum, "", EbSite.Base.Host.Instance.GetSiteID);

                //List<EbSite.Entity.TagKey> lst =  DbProviderCms.GetInstance().TagKey_GetListArr(sWhere, 3, "", EbSite.Base.Host.Instance.GetSiteID, 5);//内容至少有5条

                foreach (var model in lst)
                {
                    string sKey = model.SpecialName;
                    string sUrl = EbSite.Base.Host.Instance.GetSpecialHref(model.id, 1);
                    string sTips = model.SpecialName;
