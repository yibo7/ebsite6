function InitFileUpload(ext, folder, userid, sizeSingle, valstr, filedata, fileList, inputid, isBatch) {
    ...
    var uploader = WebUploader.create({
        ...
        server: SiteConfigs.UrlIISPath+'ajaxget/upsinglefile.ashx?tp=1&bdup=1&folder=' + folder + '&userid=' + userid + '&valstr=' + valstr + '&sz=' + sizeSingle + '&ext=' + ext,
    ...
    uploader.on('uploadSuccess', function (file, response) {
        //var data = Object;
        //try { data = eval('(' + response + ')'); } catch (ex) { };
        //if (data.isok) {
        //    $('#' + file.id).find('p.state').text('上传成功');
        //} else {
        //    $('#' + file.id).find('p.state').text("出错:" + data.msg);
        //}

        var data = Object;
        try { data = eval('(' + response._raw + ')'); } catch (ex) { };
        var uploadUrl = data.msg.url;
        //if (isBatch) {
