function ubb2html(c) {
    function g(b) { return null != b && "" != b ? !isNaN(b) : !1 } var b, l = [], e = 0, m = "10px,13px,16px,18px,24px,32px,48px".split(","); b = ("" + c).replace(/[<>&"]/g, function (b) { return { "<": "&lt;", ">": "&gt;", "&": "&amp;", '"': "&quot;"}[b] }); b = b.replace(/\r?\n/g, "<br />"); b = b.replace(/\[code\s*(?:=\s*([^\]]+?))?\]([\s\S]*?)\[\/code\]/ig, function (b) { e++; l[e] = b; return "[\tubbcodeplace_" + e + "\t]" }); b = b.replace(/\[(\/?)(b|u|i|s|sup|sub)\]/ig, "<$1$2>"); b = b.replace(/\[color\s*=\s*([^\]"]+?)(?:"[^\]]*?)?\s*\]/ig,
...
'<p align="$1">$2</p>'); b = b.replace(/\[img\]\s*(((?!")[\s\S])+?)(?:"[\s\S]*?)?\s*\[\/img\]/ig, '<img src="$1" alt="" />'); b = b.replace(/\[img\s*=([^,\]]*)(?:\s*,\s*(\d*%?)\s*,\s*(\d*%?)\s*)?(?:,?\s*(\w+))?\s*\]\s*(((?!")[\s\S])+?)(?:"[\s\S]*)?\s*\[\/img\]/ig, function (b, a, c, j, e, q) { b = '<img src="' + q + '" alt="' + a + '"'; e = e ? e : !g(c) ? c : ""; g(c) && (b += ' width="' + c + '"'); g(j) && (b += ' height="' + j + '"'); e && (b += ' align="' + e + '"'); return b + " />" }); b = b.replace(/\[emot\s*=\s*([^\]"]+?)(?:"[^\]]*?)?\s*\/\]/ig, '<img emot="$1" />');
    ...
    b = b.replace(/\[flash\s*(?:=\s*(\d+)\s*,\s*(\d+)\s*)?\]\s*(((?!")[\s\S])+?)(?:"[\s\S]*?)?\s*\[\/flash\]/ig, function (b, a, c, e) { a || (a = 480); c || (c = 400); return '<embed type="application/x-shockwave-flash" src="' + e + '" wmode="opaque" quality="high" bgcolor="#ffffff" menu="false" play="true" loop="true" width="' + a + '" height="' + c + '"/>' }); b = b.replace(/\[media\s*(?:=\s*(\d+)\s*,\s*(\d+)\s*(?:,\s*(\d+)\s*)?)?\]\s*(((?!")[\s\S])+?)(?:"[\s\S]*?)?\s*\[\/media\]/ig, function (b, a, c, e, g) {
        a || (a = 480); c || (c = 400); return '<embed type="application/x-mplayer2" src="' +
g + '" enablecontextmenu="false" autostart="' + ("1" == e ? "true" : "false") + '" width="' + a + '" height="' + c + '"/>'
    }); b = b.replace(/\[table\s*(?:=\s*(\d{1,4}%?)\s*(?:,\s*([^\]"]+)(?:"[^\]]*?)?)?)?\s*\]/ig, function (b, a, c) { b = "<table"; a && (b += ' width="' + a + '"'); c && (b += ' bgcolor="' + c + '"'); return b + ">" }); b = b.replace(/\[tr\s*(?:=\s*([^\]"]+?)(?:"[^\]]*?)?)?\s*\]/ig, function (b, a) { return "<tr" + (a ? ' bgcolor="' + a + '"' : "") + ">" }); b = b.replace(/\[td\s*(?:=\s*(\d{1,2})\s*,\s*(\d{1,2})\s*(?:,\s*(\d{1,4}%?))?)?\s*\]/ig, function (b,
...
function html2ubb(c) {
    function g(a, c, h, d) { if (!h) return d; var a = [], c = [], f; if (f = h.match(/ face\s*=\s*"\s*([^"]+)\s*"/i)) a.push("[font=" + f[1] + "]"), c.push("[/font]"); if (f = h.match(/ size\s*=\s*"\s*(\d+)\s*"/i)) a.push("[size=" + f[1] + "]"), c.push("[/size]"); if (f = h.match(/ color\s*=\s*"\s*([^"]+)\s*"/i)) a.push("[color=" + b(f[1]) + "]"), c.push("[/color]"); return a.join("") + d + c.join("") } function b(a) {
        var b; if (b = a.match(/\s*rgb\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*\)/i)) {
            for (a = (65536 * b[1] + 256 * b[2] + 1 * b[3]).toString(16); 6 >
a.length; ) a = "0" + a; a = "#" + a
        } return a = a.replace(/^#([0-9a-f])([0-9a-f])([0-9a-f])$/i, "#$1$1$2$2$3$3")
    } var l = /\s+src\s*=\s*(["']?)\s*(.+?)\s*\1(\s|$)/i, e = /\s+width\s*=\s*(["']?)\s*(\d+(?:\.\d+)?%?)\s*\1(\s|$)/i, m = /\s+height\s*=\s*(["']?)\s*(\d+(?:\.\d+)?%?)\s*\1(\s|$)/i, n = /(?:background|background-color|bgcolor)\s*[:=]\s*(["']?)\s*((rgb\s*\(\s*\d{1,3}%?,\s*\d{1,3}%?\s*,\s*\d{1,3}%?\s*\))|(#[0-9a-f]{3,6})|([a-z]{1,20}))\s*\1/i, a, o = [], j = 0; a = ("" + c).replace(/\s*\r?\n\s*/g, ""); a = a.replace(/<(script|style)(\s+[^>]*?)?>[\s\S]*?<\/\1>/ig,
...
function (a, b, c, d) { if (!c || !d) return ""; a = "url"; c.match(/^mailto:/i) && (a = "email", c = c.replace(/mailto:(.+?)/i, "$1")); b = "[" + a; c != d && (b += "=" + c); return b + "]" + d + "[/" + a + "]" }); a = a.replace(/<img(\s+[^>]*?)\/?>/ig, function (a, b) {
    var c = b.match(/\s+emot\s*=\s*(["']?)\s*(.+?)\s*\1(\s|$)/i); if (c) return "[emot=" + c[2] + "/]"; var c = b.match(l), d = b.match(/\s+alt\s*=\s*(["']?)\s*(.*?)\s*\1(\s|$)/i), f = b.match(e), i = b.match(m), k = b.match(/\s+align\s*=\s*(["']?)\s*(\w+)\s*\1(\s|$)/i), g = "[img"; if (!c) return ""; d = "" + d[2]; if (f || i) d +=
"," + (f ? f[2] : "") + "," + (i ? i[2] : ""); k && (d += "," + k[2]); d && (g += "=" + d); return g += "]" + c[2] + "[/img]"
    ...
    a = a.replace(/<embed((?:\s+[^>]*?)?(?:\s+type\s*=\s*"\s*application\/x-mplayer2\s*"|\s+classid\s*=\s*"\s*clsid:6bf52a52-394a-11d3-b153-00c04f79faa6\s*")[^>]*?)\/?>/ig, function (a, b) { var c = b.match(l), d = b.match(e), f = b.match(m), i = b.match(/\s+autostart\s*=\s*(["']?)\s*(.+?)\s*\1(\s|$)/i), g = "[media", j = "0"; if (!c) return ""; i && "true" == i[2] && (j = "1"); d && f && (g += "=" + d[2] + "," + f[2] + "," + j); g += "]" + c[2]; return g + "[/media]" }); a = a.replace(/<table(\s+[^>]*?)?>/ig, function (a, b) {
        var c = "[table"; if (b) {
            var d = b.match(e), f = b.match(n);
            d && (c += "=" + d[2], f && (c += "," + f[2]))
        } return c + "]"
    }); a = a.replace(/<tr(\s+[^>]*?)?>/ig, function (a, b) { var c = "[tr"; if (b) { var d = b.match(n); d && (c += "=" + d[2]) } return c + "]" }); a = a.replace(/<(?:th|td)(\s+[^>]*?)?>/ig, function (a, b) { var c = "[td"; if (b) { var d = b.match(/\s+colspan\s*=\s*(["']?)\s*(\d+)\s*\1(\s|$)/i), f = b.match(/\s+rowspan\s*=\s*(["']?)\s*(\d+)\s*\1(\s|$)/i), g = b.match(e), d = d ? d[2] : 1, f = f ? f[2] : 1; if (1 < d || 1 < f || g) c += "=" + d + "," + f; g && (c += "," + g[2]) } return c + "]" }); a = a.replace(/<\/(table|tr)>/ig, "[/$1]"); a = a.replace(/<\/(th|td)>/ig,
