/* 
<select id="birthday" drp="1" ">
drp 如果可以获取，将会在鼠标经过时下拉
$(birthday).click() 将可激活一个下拉
jQuery Select(单选) 模拟插件 V1.3.62 改进版 在此基础上做了一些修改
*/
        ...
        sSelect: function (vid, tid) {
            ...
            return this.each(function (i, obj) {
                ...
                if (obj.style.display != 'none' && $(this).parents()[0].id.indexOf('__jQSelect') < 0) {
                    ...
                    var subselectWidth = $('#' + selectId + ' select option').width();
                    $('#' + selectId + ' ul').css({ width: subselectWidth > 10 ? subselectWidth : selectUlwidth + 'px' });
                    ...
                    var click_fun = function () {
                        ...
                        if ($.browser.msie && ($.browser.version == "6.0") && !$.support.style) { //兼容IE6
                            var _dtitle = $('#' + selectId + ' h4');
                            $('#' + selectId + ' ul').css({ left: _dtitle.offset().left - _dtitle.outerWidth()-20 });
                        } 
...
.bind("change", function () {
    var obtxt = this.options[this.selectedIndex].text;
    var obval = this.options[this.selectedIndex].value;
    ...
    return false;
        ...
        setSelectValue: function (sID) {
            ...
            $.each($('#' + sID + ' option'), function (i) {
                content.push("<li class='FixSelectBrowser' sid=\""+$(this).attr("id")+"\">" + $(this).text() + "</li>");
            });
