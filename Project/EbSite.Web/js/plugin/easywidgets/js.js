    ...
    */
    function WidgetsPositionsChange(settings) {
        ...
        $(s.selectors.places).each(function () {
            var widgets = '';
            var place = $(this);
            var places = place.attr('id').replace("ctl00_", "") + '='; //防止asp.net输出后客户端ID发生变化，这里自定义 place.attr('cid')
            place.children(s.selectors.widget).each(function () {
                var widget = this;
                var widgetId = widget.id + "*" + $(widget).attr("twid"); //cqs
                var haveId = ($.trim(widgetId) != '');
                if (haveId) {
                    if (widgets == '') {
                        widgets += widgetId;
...
var EASettings = {
    ...
    i18n: {
        editText: '<img src="' + SiteConfigs.UrlIISPath + 'themes/edit.png" alt="编辑" width="16" height="16" />',
        closeText: '<img src="' + SiteConfigs.UrlIISPath + 'themes/close.png" alt="关闭" width="16" height="16" />',
        collapseText: '<img src="' + SiteConfigs.UrlIISPath + 'themes/collapse.png" alt="收回" width="16" height="16" />',
        cancelEditText: '<img src="' + SiteConfigs.UrlIISPath + 'themes/edit.png" alt="取消编辑" width="16" height="16" />',
        extendText: '<img src="' + SiteConfigs.UrlIISPath + 'themes/extend.png" alt="展开" width="16" height="16" />'
    },
    effects: {
        effectDuration: 200,
        widgetShow: 'slide',
    ...
    callbacks: {
        ...
        onEdit: function (link, widget) {
            var sType = widget.attr("t");
            var sID = widget.attr("id");

            var sUrl = SiteConfigs.UrlIISPath + "home/WidgetEditor.aspx?id=" + sID + "&t=" + sType;
            //IsColseReLoadPage = 1;
            OpenDialog_Iframe(sUrl, "编辑部件", 600, 600);
            return true;
        },

        onEditQuery: function (link, widget) {
            return true;
            //return confirm('onEditQuery')
        },
        ...
        onCancelEditQuery: function (link, widget) {
            return confirm('onCancelEditQuery')
        },
        onClose: function (link, widget) {
            var sID = widget.attr("twid");

            var tabid = GetUrlParams("tab");
            var MarkT = GetUrlParams("t");
             var widgetid = widget.attr("id");
             var Url = SiteConfigs.UrlIISPath + "home/ajaxget/DeleteTabWidget.ashx?wdid=" + widgetid + "&tid=" + tabid + "&twid=" + sID + "&t=" + MarkT + "&i=" + Math.random();
                run_ajax_async(Url, "", null);

        },
        onChangePositions: function (str) {

            var Url = SiteConfigs.UrlIISPath + "home/ajaxget/WidgetPosChange.ashx?zd=" + str + "&i=" + Math.random();
            run_ajax_async(Url, "", null);
...
function AddWidget(html) {
    ...
    $.fn.AddEasyWidget(html, "wcl", EASettings);
}
