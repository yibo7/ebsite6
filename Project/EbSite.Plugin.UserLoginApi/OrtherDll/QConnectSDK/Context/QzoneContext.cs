...
namespace QConnectSDK.Context
{
    ...
    public class QzoneContext
    {
        ...
        /// <param name="callBackURL">登录成功后的URL</param>
        public QzoneContext(string appID,string appKey,string callBackURL)
        {
            this.config = new QQConnectConfig();
            this.config.AppKey = appID;
            this.config.AppSecret = appKey;
        ...
        /// <param name="state"></param>
        /// <returns></returns>
        public OAuthToken GetAccessToken(Config.QQConnectConfig config, string state)
        {
            oAuthToken = restApi.GetAccessToken(config,oAuthVericode, state);
            return oAuthToken;
        }
