...
namespace QConnectSDK
{
    ...
    public class QOpenClient
    {
        ...
        public QOpenClient(Config.QQConnectConfig config, string verifierCode, string state)
        {
            var context = new QzoneContext(verifierCode);
            if (!string.IsNullOrEmpty(verifierCode))
            {
                this.OAuthToken = context.GetAccessToken(config, state); 
            }
        ...
        /// </summary>
        /// <param name="oAuthToken"></param>
        public QOpenClient(OAuthToken oAuthToken, Config.QQConnectConfig config)
        {
            this._oAuthToken = oAuthToken;
            var context = new QzoneContext(oAuthToken);
