...
namespace EbSite.Data.User.SqlServer
{
    ...
    public partial class DataProviderUser : Interface.IDataProviderUser
	{
		...
		public List<Entity.Coupons> Coupons_GetListPages(int PageIndex, int PageSize, string strWhere, string Fileds, string oderby,out int RecordCount)
		{
            ...
            if(strWhere=="1")
            {
                strWhere = "SentCount>0 and EndDateTime>GETDATE()";
            }
            if(strWhere=="2")
            {
                strWhere = "EndDateTime<GETDATE()";
            }
            ...
            List<Entity.Coupons> list = new List<Entity.Coupons>();
            using (IDataReader dataReader = SplitPages.GetListPages_SP(DbHelperUser.Instance, "Coupons", PageSize, PageIndex, Fileds, "id", oderby, sbSql.ToString(), out RecordCount, sPre))
            {
                while (dataReader.Read())
                {
                    list.Add(Coupons_ReaderBind(dataReader));
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.AppendFormat("select {0} from eb_coupons where id in(SELECT couponid from eb_couponitems where UserId={1} GROUP BY CouponId) and enddatetime>getdate()",sFieldCoupons ,uid );
            List<Entity.Coupons> list = new List<Entity.Coupons>();
            using (IDataReader dataReader = DbHelperUser.Instance.ExecuteReader(CommandType.Text, strSql.ToString()))
            {
                while (dataReader.Read())
