...
namespace EbSite.Data.User.SqlServer
{
    ...
    public partial class DataProviderUser : Interface.IDataProviderUser
	{
		...
		{
            List<Entity.OrderOptions> list = new List<Entity.OrderOptions>();
            using (IDataReader dataReader = SplitPages.GetListPages_SP(DbHelperUser.Instance, "OrderOptions", PageSize, PageIndex, Fileds, "id", oderby, strWhere, out RecordCount, sPre))
            {
                while (dataReader.Read())
                {
                    list.Add(OrderOptions_ReaderBind(dataReader));
