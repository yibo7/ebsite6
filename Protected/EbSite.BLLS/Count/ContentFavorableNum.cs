...
namespace EbSite.BLL.Count
{
    ...
    /// 添加内容收藏数
    /// </summary>
    public class ContentFavorableNum : ContentBase
    {
        public override string sCacheKey  //每类统计要重写不同的sCacheKey
        {
            get
            {
                return string.Concat("CountFavorableNum", TableName);
            }
        }
        public ContentFavorableNum(int _ClassID)
            : base(_ClassID)
        {
         
        }
        public static ContentFavorableNum Instance(int ClassID)
        {
            //可以优化，减少创建
            return new ContentFavorableNum(ClassID);
        }
        override protected object AddToDb(object model)
        {
            ...
            if (!Equals(de, null) && !string.IsNullOrEmpty(de.Key.ToString()))
                NewsContentInst.AddFavorableNum(int.Parse(de.Key.ToString()), int.Parse(de.Value.ToString()));
