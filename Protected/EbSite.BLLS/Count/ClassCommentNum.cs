...
namespace EbSite.BLL.Count
{
    ...
    /// 添加评论数
    /// </summary>
    public class ClassCommentNum : CountBase
    {
        public static readonly ClassCommentNum Instance = new ClassCommentNum();
        public override string sCacheKey  //每类统计要重写不同的sCacheKey
        {
            get
            {
                return "CountClassCommentNum";
            }
        }
        public ClassCommentNum()
        {
        ...
        override protected object AddToDb(object model)
        {
            ...
            if (!Equals(de, null) && !string.IsNullOrEmpty(de.Key.ToString()))
                NewsClass.AddCommentNum(int.Parse(de.Key.ToString()), int.Parse(de.Value.ToString()));
