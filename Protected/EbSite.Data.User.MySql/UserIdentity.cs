    ...
    public partial class DataProviderUser : Interface.IDataProviderUser
    {
        ...
        public void WriteUserIdentity(string UserID, string UserName, string UserNiName, string UserPass, int ExpiresTime, string RoleID)
       {
           if (!string.IsNullOrEmpty(UserName) && !string.IsNullOrEmpty(UserNiName) && !string.IsNullOrEmpty(UserPass))
           {
               ...
               cookie.Values[sUserPassMark] = Utils.UrlEncode(EbSite.Base.Host.Instance.EncodeByKey(UserPass));
               cookie.Values[sUserRoleIDMark] =Utils.UrlEncode(Base.Host.Instance.EncodeByKey(RoleID));
       ...
       public string GetUserPass()
        {
            ...
            {
                string sKey = Base.Configs.SysConfigs.ConfigsControl.Instance.EncryptionKey;
       ...
       {
           string sid = EbSite.Core.Utils.GetSingleVlue(sCookieHeader_User, sUserRoleIDMark);

           if (!string.IsNullOrEmpty(sid))
           {
              
               return int.Parse(Utils.UrlDecode(Base.Host.Instance.DecodeByKey(sid)));
           }
           else
           {
               return 0;
