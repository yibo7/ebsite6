...
namespace EbSite.Base.Extension.Manager
{
    ...
    public class ExtensionSettings
    {
        ...
        public ExtensionSettings(object extension)
        {
            ...
            //默认添加一个id键,以便查询
            this.AddParameter("ID", "唯一ID", 20, true, true,ParameterType.String);
        ...
        /// 添加表单
        /// </summary>
        /// <param name="name">键值，相当于表的字段名称</param>
        /// <param name="label">展示时的标题</param>
        /// <param name="maxLength">展示控件的大小值</param>
        /// <param name="required">这个控件的值是不是必填</param>
        /// <param name="keyfield">将字段标记为主键、唯一且必需</param>
        /// <param name="parType">展示的控件类型</param>
        ...
        /// </summary>
        /// <returns></returns>
        public System.Data.DataRow GetRowByID(string sID)
        {
            System.Data.DataTable dt = GetDataTable();

            System.Data.DataRow[] drs = dt.Select("ID='" + sID + "'");

            if(!Equals(drs,null)&&drs.Length>0)
            {
                return drs[0];
            }
            else
            {
                throw new AbandonedMutexException("找不到对应的记录");
            }
        }
        /// <summary>
