        ...
        private static string GetBaseConfigsPath
        {
            get
            {
                if (filename == null)
                {
                    ...
                    {
                        filename = HttpContext.Current.Server.MapPath("~/ConfigsFile/SysConfig.config");
                    }
                    else
                    {
                        filename = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "ConfigsFile\\SysConfig.config");
                    }
