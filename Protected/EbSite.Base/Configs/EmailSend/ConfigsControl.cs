        ...
        private static string GetBaseConfigsPath
        {
            get
            {
                if (filename == null)
                {
                    ...
                    {
                        filename = HttpContext.Current.Server.MapPath("~/ConfigsFile/EmailSend.config");
                    }
                    else
                    {
                        filename = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "ConfigsFile\\EmailSend.config");
                    }
