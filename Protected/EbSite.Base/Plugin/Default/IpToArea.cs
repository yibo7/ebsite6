...
{
    [Extension("从IP获取区域之默认", "1.0", "<a href=\"http://www.ebsite.net\">小菜菜</a>")]
    public class IpToArea : IIPToArea
    {
        public ClientIpInfo Query(string strIP)
        {
            EbSite.Entity.ClientIpInfo ipModel = new ClientIpInfo();
            ipModel = null;
            if (!string.IsNullOrEmpty(strIP))
            {
                try
                {
                    ...
                    ipModel = JsonHelper.Deserialize<EbSite.Entity.ClientIpInfo>(strJson);
