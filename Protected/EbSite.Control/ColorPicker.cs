...
namespace EbSite.Control
{
    ...
    {
        private TextBox _ColorValue = new TextBox();
        private string sPath = string.Format("{0}js/plugin/colorpicker/", Base.AppStartInit.IISPath);

        protected override void CreateChildControls()
        {
            this.Controls.Clear();
            this._ColorValue.ID = "ColorPk";
            this.Controls.Add(this._ColorValue);
        }

        public bool LoadPostData(string postDataKey, NameValueCollection postCollection)
        {
            string text = this._ColorValue.Text;
        ...
        protected override void OnPreRender(EventArgs e)
        {
            ...
            builder.AppendFormat("<link rel=\"stylesheet\" href=\"{0}themes/index.css\" type=\"text/css\" />", this.sPath);
            builder.AppendFormat("<script type=\"text/javascript\" src=\"{0}colorpicker.js\"></script>", this.sPath);
            //builder.AppendFormat("<script type=\"text/javascript\" src=\"{0}eye.js\"></script>", this.sPath);
            if (!this.Page.ClientScript.IsClientScriptBlockRegistered("colorpicker"))
            {
                this.Page.ClientScript.RegisterClientScriptBlock(base.GetType(), "colorpicker", builder.ToString());
            }
            base.OnPreRender(e);
        }
        ...
        {
            this._ColorValue.Width = base.Width;
            this._ColorValue.Height = base.Height;
            this._ColorValue.Attributes.Add("style", string.Format("background-color:#{0}", this.Color));
            this._ColorValue.RenderControl(output);
            output.WriteLine("<script>");
            output.WriteLine("(function($){");

            output.Write(string.Concat("(function ($) { $('#", this._ColorValue.ClientID, "').ColorPicker({ onSubmit: function (hsb, hex, rgb, el) { $(el).val(hex); $(el).ColorPickerHide(); }, onBeforeShow: function () { $(this).ColorPickerSetColor(this.value); }, onChange: function (hsb, hex, rgb) { $('#", this._ColorValue.ClientID, "').css('backgroundColor', '#' + hex); $('#", this._ColorValue.ClientID, "').val(hex); } }); })(jQuery);"));

            output.WriteLine("})(jQuery)");
            output.Write("</script>");

            //output.WriteLine("<script>");
            //output.WriteLine("var obCur" + this._ColorValue.ClientID + " = document.getElementsByName(\"" + this._ColorValue.ClientID + "\");");
            //output.WriteLine("(function($){");
            //output.WriteLine("var initLayout = function() {");
            //output.WriteLine("$(obCur" + this._ColorValue.ClientID + ").ColorPicker({");
            //output.WriteLine("onSubmit: function(hsb, hex, rgb, el) {");
            //output.WriteLine("$(el).val(hex);");
            //output.WriteLine("$(el).ColorPickerHide();},");
            //output.WriteLine("onBeforeShow: function () {");
            //output.WriteLine("$(this).ColorPickerSetColor(this.value);");
            //output.WriteLine("}");
            //output.WriteLine(",");
            //output.WriteLine("onChange: function (hsb, hex, rgb) {");
            //output.WriteLine("$(obCur" + this._ColorValue.ClientID + ").css('backgroundColor', '#' + hex);");
            //output.WriteLine("$(obCur" + this._ColorValue.ClientID + ").val(hex);");
            //output.WriteLine("}");
            //output.WriteLine("})");
            //output.WriteLine("};");
            //output.WriteLine("var showTab = function(e) {");
            //output.WriteLine("var tabIndex = $('ul.navigationTabs a')");
            //output.WriteLine(".removeClass('active')");
            //output.WriteLine(".index(this);");
            //output.WriteLine("$(this)");
            //output.WriteLine(".addClass('active')");
            //output.WriteLine(".blur();");
            //output.WriteLine("$('div.tab')");
            //output.WriteLine(".hide()");
            //output.WriteLine(".eq(tabIndex)");
            //output.WriteLine(".show();");
            //output.WriteLine("};");
            //output.WriteLine("EYE.register(initLayout, 'init');");
            //output.WriteLine("})(jQuery)");
            //output.Write("</script>");
        }

        [DefaultValue("ff0000"), Description("颜色值")]
        public string Color
        {
            get
            {
                return this._ColorValue.Text;
            }
            set
            {
                this._ColorValue.Text = value;
            }
        }
    }
...
//    public class ColorPicker : EbSite.Control.WebControl, IPostBackDataHandler,
...
//        /// 拾取文本框
//        /// </summary>
//        protected EbSite.Control.TextBox ColorTextBox = new EbSite.Control.TextBox();
...
//                else
//                {
//                    return GetVirPath + "themes/colorpicker.gif";
//                }
//            }
//            set
//            {
...
//                return obj == null ? GetVirPath+"js/colorpicker.js" : (string)obj;
//            }
//            set
//            {
...
//                return obj == null ? GetVirPath+"themes/colorpicker.css" : (string)obj;
//            }
//            set
//            {
