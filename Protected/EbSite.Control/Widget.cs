...
namespace EbSite.Control
{
    public class Widget : UserControl
    {
        ...
        private ETimeSpanModel _TimeSpanModel = ETimeSpanModel.T;
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (!Equals(mdWidget, null))
            {
                ...
                { 
                    ...
                    if (CacheTimeSpan > 0)
                    {
                        //string html = EbSite.Base.Plugin.Factory.GetSTSdbCache().GetCacheItem<string>(sKey, "Widget");
                        string html = EbSite.Base.Host.CacheApp.GetCacheItem<string>(sKey, "Widget");
                    ...
                    if (isloadctr)
                    {
                        ...
                        if (CacheTimeSpan > 0)
                        {
                            ...
                            EbSite.Base.Host.CacheApp.AddCacheItem(sKey, writer.InnerWriter.ToString(), CacheTimeSpan, CacheTimeSpanModel, "Widget");
                            //EbSite.Base.Plugin.Factory.GetSTSdbCache().AddCacheItem(sKey, writer.InnerWriter.ToString(), CacheTimeSpan, CacheTimeSpanModel, "Widget");
                        }
                ...
                else
                {
                    ...
                    EbSite.Log.Factory.GetInstance().ErrorLog(string.Format("找不到部件:{0},来源:{1}", WidgetID,Request.RawUrl));
                }
